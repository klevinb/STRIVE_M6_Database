...
router.post(
  ...
  [
    ...
    check("elementId").exists().withMessage("Insert product ID please!"),
  ],
  async (req, res, next) => {
    try {
      ...
      if (!errors.isEmpty()) {
        ...
        const reviewAt = await ProductModel.addReviewToProduct(
          req.body.elementId,
          _id
        );
...
router.delete("/:id", async (req, res, next) => {
  try {
    ...
    if (findReview) {
      ...
      await ProductModel.removeReviewFromProduct(
        findReview.elementId,
        req.params.id
      );
