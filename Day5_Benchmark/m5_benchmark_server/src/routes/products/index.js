...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    if (findProduct) {
      ...
      const addProduct = await ProductModel.findByIdAndUpdate(
        req.params.id,
        updatedProduct
      );
...
router.delete('/:id', async (req, res, next) => {
  try {
    ...
    if (findProduct) {
      ...
      fs.unlink(path.join(imagePath, `${req.params.id}.png`), (err) => {
        if (err) {
          console.log(err);
        }
...
router.get('/:id/exportToPDF', async (req, res, next) => {
  try {
    ...
    if (findProduct) {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename=${findProduct._id}.pdf`
      );
...
router.get('/convert/exportToCSV', async (req, res, next) => {
  ...
  await fs.writeJSON(path.join(productsPath, 'products.json'), products);
  if (products.length > 0) {
    try {
      ...
      const input = fs.createReadStream(
        path.join(productsPath, 'products.json'),
        { encoding: 'utf8' }
      );
