...
class App extends Component {
  ...
  getUserInfo = async () => {
    const getUserInfo = await fetch(apiKey + "/cart/5f086c7ad968173410b95179");
    if (getUserInfo.ok) {
  ...
  addToCart = async (id) => {
    ...
    const addToCart = await fetch(
      apiKey + "/cart/5f086c7ad968173410b95179/add-product-to-cart/" + id,
      {
        method: "POST",
  ...
  removeProduct = async (id) => {
    console.log(id);
    const resp = await fetch(apiKey + "/cart/" + id, {
      method: "DELETE",
    });
    if (resp.ok) this.getUserInfo();
  };
  ...
  calculateTotal = async () => {
    const total = this.state.cart
      .map((product) => product.unitary_price * product.quantity)
              ...
              {this.state.cart.length > 0 ? (
                this.state.cart.map((item) => (
                  <div className='d-flex justify-content-between text-center'>
                    ...
                    <div>
                      Price:<br></br> {item.unitary_price} $
                    </div>
                    <div>
                      Total for product:<br></br> {item.total} $
                    </div>
                  </div>
                ))
              ) : (
